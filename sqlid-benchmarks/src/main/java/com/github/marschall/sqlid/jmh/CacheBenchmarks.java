...
public class CacheBenchmarks {
  ...
  public void getInCapacitySpring(Blackhole blackhole) {
    for (int i = 0; i < CAPACITY; i++) {
      for (int j = 0; j < i; j++) {
        blackhole.consume(this.springCache.get(j));
      }
    }
  }
  ...
  public void getInCapacityCaffeine(Blackhole blackhole) {
    for (int i = 0; i < CAPACITY; i++) {
      for (int j = 0; j < i; j++) {
        blackhole.consume(this.caffeineCache.get(j, Function.identity()));
      }
    }
  }
  ...
  public void getInCapacityProject(Blackhole blackhole) {
    for (int i = 0; i < CAPACITY; i++) {
      for (int j = 0; j < i; j++) {
        blackhole.consume(this.projectCache.get(j, Function.identity()));
      }
    }
  }
